...
# R script for Random Forest model to predict OM2 vs REF
...
deseq_results<- filter(deseq_om2_ref,
...
# CLR transformation
ps_binary <- subset_samples(phylo_soil_genus, LTSP.Treatment %in% c("REF", "OM2")) 
...
df_pivot = df_transformed %>% 
  select(Sample,pH,Total.Carbon,Moisture.Content,Total.Nitrogen, pH, CN.Ratio,LTSP.Treatment,Genus,Abundance) %>% 
...
save(soil_model,file = "output/soil_model_REF_OM2.Rdata")



####Figures####

load("output/soil_model_REF_OM2.Rdata")
...
roc_data = data.frame(Dataset = 'RF OM2 REF Data',
...
#### Save plots ####
roc_plot
ggsave("figures/om2_ref_roc.png",
       units=c("in"),
       width = 4,
       height = 3)
importance_plot
ggsave("figures/om2_ref_importance.png",
       units=c("in"),
       width = 6,
       height = 3)
