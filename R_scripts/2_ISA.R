...
# Select the top 10 indicator taxa for each ecozone based on indicator value
plot_df <- res %>%
  group_by(Ecozone) %>%
...
# Create the plot
p <- ggplot(plot_df, aes(x = reorder(taxon, stat), y = stat, fill = Ecozone)) +
  geom_col() +
  coord_flip() +
  facet_wrap(~Ecozone, scales = "free") +
  labs(
    title = "Top indicator taxa by ecozone",
    x = "Taxon",
    y = "Indicator value"
  ) +
  theme_classic() +
...
# Save the figure
ggsave("ISA_indicator_plot.png", plot = p, width = 10, height = 6)
